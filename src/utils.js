...
export const validateCreation = () => {
  const iosInfoPlistFullPath = globbySync(normalizePath(path.join(APP_PATH, iosAppDelegate)))[0];
  const fileExists =
...
export const getIosCurrentName = () => {
  const filepath = globbySync(normalizePath(path.join(APP_PATH, iosAppDelegate)))[0];
...
export const updateIosNameInInfoPlist = async newName => {
  await updateElementInXml({
    filepath: globbySync(normalizePath(path.join(APP_PATH, iosAppDelegate)))[0],
